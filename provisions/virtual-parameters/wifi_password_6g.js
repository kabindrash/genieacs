// Virtual Parameter: wifi_password_6g
// Vendor-agnostic read/write access to the 6GHz WiFi password.
// Discovers the 6GHz radio dynamically — no hardcoded instance numbers.
// Handles ZTE vs standard password path differences automatically.
// Returns empty string if device has no 6GHz radio (graceful degradation).
...
if (isTR181) {
  ...
  for (let r of radios) {
    ...
    if (fb.value && fb.value[0] && (fb.value[0] === "6GHz" || (fb.value[0].indexOf("6") >= 0 && fb.value[0].indexOf("60") < 0))) {
  ...
  for (let w of wlans) {
    ...
    if (classifyBand(chVal, possStr) === "6") {
      found = w.path;
      break;
    }
  }
